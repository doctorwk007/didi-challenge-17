...
int MultiSmartTracker::addTracker(const cv::Mat &image, perception_msgs::Obstacle obs_msg){
...
void MultiSmartTracker::update_prediction(cv::Mat frame, const perception_msgs::ObstacleList::ConstPtr& obs_msg, pcl::PointCloud<pcl::PointXYZ>::Ptr radar_cloud){
    ...
    for (int i = 0; i < trackers_.size(); ++i){
        ...
        if(tracked_center.x > 0 && tracked_center.x < 700 && tracked_center.y > 0 && tracked_center.y<700){ // Avoid prediction of out of range radar agents
            ...
            if(!success){
                ...
                std::cout << "Updating tracker with prediction" << std::endl;
            }
        }else{
            to_remove.push_back(i);
        }
    ...
    for (int j = 0; j < radar_cloud->points.size(); ++j){
        radar_map[j]=true;
        cv::Point2d radar2d;
        radar2d.x = -(radar_cloud->points[j].y/cell_size_)+grid_dim_/2/cell_size_;
        radar2d.y = -(radar_cloud->points[j].x/cell_size_)+grid_dim_/2/cell_size_;

        tracks_.push_back(radar2d);
    }


    // Match radar tracks to agents
//    cout << "Matching agents " << trackers_.size() << endl;
    for (int i = 0; i < trackers_.size(); ++i){
        vector<cv::Point2d> radars;
        for (int j = 0; j < radar_cloud->points.size(); ++j){
            cv::Point2d radar2d;
            radar2d.x = -(radar_cloud->points[j].y/cell_size_)+grid_dim_/2/cell_size_;
            radar2d.y = -(radar_cloud->points[j].x/cell_size_)+grid_dim_/2/cell_size_;
            if(trackers_[i].isRadarInROI(radar2d)){
                radars.push_back(radar2d);
                // TODO SET AS MATCHED
                radar_map[j]=false;
            }
        }
        trackers_[i].associate_radars(radars);
    }

    double current_threshold = 0.95;
    int unassigned_agents = tracker_map.size();
    int unassigned_detections = detection_map.size();
    while(unassigned_agents!=0 && current_threshold>0.2){ // Do Not consider under 0.6 scored detections
//        std::cout << unassigned_agents << " agents not updated yet: " << std::endl;
...
//                    cout << "New center " << tracked_center << endl;
    ...
    for (std::map<int,bool>::iterator it=tracker_map.begin(); it!=tracker_map.end(); ++it){
        if(!tracker_map[it->first]) continue;
            if(std::find(to_remove.begin(), to_remove.end(), it->first) != to_remove.end()){ // CHeck for radar if image does not work
                cout << "Prediction and detection for agent " << trackers_[it->first].getid() << "failed, checking now with radars: " << radar_map.size() << endl;
                tracker_map[it->first] = !trackers_[it->first].update_from_radar();
...
//                }
    ...
    for (std::map<int,bool>::iterator it_d=detection_map.begin(); it_d!=detection_map.end(); ++it_d){
        ...
        if(detection_map[it_d->first] && obs_msg->obstacles[it_d->first].score > 0.2){ // TODO 0.9
            ...
            for (std::map<int,bool>::iterator it=tracker_map.begin(); should_add && it!=tracker_map.end(); ++it){
                ...
                double dy = tracked_center.y - obs_center.y;
                double current_distance = sqrt(dx*dx + dy*dy);
//                cout << "Distance to agent " << current_distance * cell_size_ << endl;
...
//                    if (current_distance * cell_size_ < 2){
                ...
                cout << "New Score " << o.score << endl;
//                if(o.score > 0.9)
                int index = addTracker(frame, o);
                tracker_map[index] = false;
            }
        }
    }
...
bool MultiSmartTracker::isRadarInROI(cv::Point2d p, cv::Rect2d roi){
    double pixel_margin = 2.5/cell_size_;
...
//        std::cout << "ROI " << tracker.get_roi() <<  std::endl;
    ...
    for (auto tracker : tracks_){
        cv::circle(image, tracker, 3, YELLOW);
    }
